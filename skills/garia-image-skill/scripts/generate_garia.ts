...
async function callGariaDraw(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // 情况 3：兼容 OpenAI 格式
      const openaiData = (data as unknown as { data?: Array<{ url?: string }> }).data;
      ...
      if (err instanceof AxiosError && err.code === 'ECONNABORTED') {
        ...
        logWarn(`  ⚠️  请求失败: ${err instanceof Error ? err.message : String(err)}`);
      }
      if (attempt < maxRetries) {
        await sleep(5000);
      }
...
async function pollGariaResult(
  ...
  while (Date.now() - startTime < maxWait * 1000) {
    try {
      ...
      let resp;
      try {
        resp = await axios.post<GariaResponse>(url, { id: taskId }, { headers, timeout: 30000 });
      } catch {
        // 尝试 GET 方式
        resp = await axios.get<GariaResponse>(`${url}?id=${taskId}`, { headers, timeout: 30000 });
      }
      ...
      if (err instanceof AxiosError && err.code === 'ECONNABORTED') {
        ...
        logWarn(`  ⚠️  轮询失败: ${err instanceof Error ? err.message : String(err)}`);
      }
      await sleep(pollInterval * 1000);
    }
  }
...
async function downloadImage(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logWarn(`  ⚠️  下载失败: ${err instanceof Error ? err.message : String(err)}`);
      if (attempt < maxRetries) {
        await sleep(3000);
      }
    }
...
function extractAspectRatioFromPrompt(prompt: string): string {
  ...
  if (lower.includes('16:9') || lower.includes('16：9') || prompt.includes('横版')) {
    return ASPECT_RATIO_MAP['16:9'];
  }
  if (lower.includes('1:1') || lower.includes('1：1') || prompt.includes('方形')) {
...
async function generateSingle(
  ...
  const imageUrl = await callGariaDraw(apiHost, apiKey, model, prompt, aspectRatio, quality, maxRetries);
...
async function main(): Promise<void> {
  ...
  program
    ...
    .option('--aspect-ratio <ratio>', '画面比例 (16:9, 1:1, 3:2, 4:3, 21:9)', '16:9')
  ...
  const apiHost = process.env.GARIA_API_HOST || process.env.API_HOST || DEFAULT_GARIA_HOST;
  ...
  if (!opts.input) {
    ...
    logInfo('   使用方式: npx ts-node generate_garia.ts --input C2-光现象/images.json --output C2-光现象/c2-images');
...
main().catch((err) => {
  logError(`❌ 运行时错误: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
