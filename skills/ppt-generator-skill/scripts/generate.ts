...
interface ContentData {
  ...
  /** 对比: 左侧 vs 右侧 */
  leftSide?: { title: string; items: string[] };
  rightSide?: { title: string; items: string[] };
  /** 图标网格: 图标名+标签+描述 */
  gridItems?: Array<{ icon: string; label: string; desc: string }>;
  /** 引用: 引文 + 作者 */
  quote?: { text: string; author: string };
  /** 卡片网格 */
  cards?: Array<{ title: string; desc: string; color?: string }>;
  /** 流程 */
...
class PPTGenerator {
  ...
  addSummaryPage(title: string | undefined, points: string[]): void {
    ...
    // 页面标题
    slide.addText(
      title || (this.options.language === "zh" ? "总结" : "Summary"),
      {
        x: 0.8,
        y: 0.5,
        w: 11.5,
  ...
  addEndPage(message?: string): void {
    ...
    // 结束语
    slide.addText(
      message || (this.options.language === "zh" ? "谢谢观看" : "Thank You"),
      {
        x: 0,
        y: 3,
        w: 13.33,
  ...
  addTwoColumnPage(title: string, leftItems: string[], rightItems: string[]): void {
    ...
    slide.addText(title, {
      ...
      fontSize: 30, bold: true, fontFace: "Microsoft YaHei",
      color: this.colors.primary,
    });

    // 中间分割线
    slide.addShape("rect", {
      x: 6.5, y: 1.3, w: 0.04, h: 5.8,
  ...
  addTimelinePage(
    title: string,
    events: Array<{ period: string; title: string; desc: string }>,
  ): void {
    const slide = this.pptx.addSlide();
    slide.background = { color: this.colors.bgLight || "FFFFFF" };
    ...
    events.forEach((evt, i) => {
      ...
      slide.addText(evt.period, {
        x: 0.2, y, w: 1.7, h: 0.4,
        fontSize: 13, bold: true, fontFace: "Microsoft YaHei",
        color: this.colors.primary, align: "right",
      });

      // 标题
      slide.addText(evt.title, {
        x: 2.8, y, w: 9.5, h: 0.4,
        fontSize: 16, bold: true, fontFace: "Microsoft YaHei",
        color: this.colors.text,
      });

      // 描述
      slide.addText(evt.desc, {
        x: 2.8, y: y + 0.4, w: 9.5, h: itemH - 0.5,
  ...
  addIconGridPage(
    ...
    items.forEach((item, idx) => {
      ...
      slide.addText(item.icon, {
        ...
        fontSize: 28, align: "center", fontFace: "Segoe UI Emoji",
      });

      // 标签
      slide.addText(item.label, {
        x: x + 0.2, y: y + 0.9, w: cardW - 0.4, h: 0.4,
        fontSize: 15, bold: true, fontFace: "Microsoft YaHei",
        color: this.colors.primary, align: "center",
      });

      // 描述
      slide.addText(item.desc, {
        x: x + 0.2, y: y + 1.3, w: cardW - 0.4, h: cardH - 1.5,
        fontSize: 11, fontFace: "Microsoft YaHei",
        color: this.colors.textLight, align: "center",
   ...
   * 引用布局 — 突出引用/名言
   */
  addQuotePage(title: string, quote: { text: string; author: string }): void {
    const slide = this.pptx.addSlide();
    slide.background = { color: this.colors.primary };

    // 大引号装饰
    slide.addText("❝", {
      x: 0.8, y: 0.8, w: 2, h: 1.5,
      fontSize: 72, fontFace: "Microsoft YaHei",
      color: this.colors.accent, transparency: 50,
    });
    ...
    slide.addText(quote.text, {
      ...
      color: "FFFFFF", lineSpacingMultiple: 1.6,
      italic: true,
    });

    // 引用线
    slide.addShape("rect", {
      x: 0.8, y: 5.8, w: 3, h: 0.04,
      fill: { color: this.colors.accent },
    });

    // 作者
    slide.addText(`—— ${quote.author}`, {
      x: 0.8, y: 6, w: 11.5, h: 0.6,
      fontSize: 16, fontFace: "Microsoft YaHei",
      color: this.colors.secondary, align: "left",
    });
    ...
    slide.addText(title, {
      ...
      fontSize: 12, fontFace: "Microsoft YaHei",
      color: this.colors.textLight, transparency: 40,
    });
  }
  ...
  addSectionHeaderPage(title: string, subtitle?: string, sectionNum?: string): void {
    ...
    if (sectionNum) {
      slide.addText(sectionNum, {
        x: 0.8, y: 0.5, w: 11.5, h: 2,
        fontSize: 96, bold: true, fontFace: "Microsoft YaHei",
        color: this.colors.accent, transparency: 80,
      });
    }
    ...
    slide.addText(title, {
      ...
      fontSize: 44, bold: true, fontFace: "Microsoft YaHei",
      color: "FFFFFF",
    });

    // 装饰线
    slide.addShape("rect", {
      x: 0.8, y: 4.8, w: 2, h: 0.05,
      fill: { color: this.colors.accent },
    });
  ...
  addCardGridPage(
    ...
    cards.forEach((card, idx) => {
      ...
      // 卡片背景
      slide.addShape("roundRect", {
        x, y: y + 0.12, w: cardW, h: cardH - 0.12,
      ...
      slide.addText(card.desc, {
        x: x + 0.3, y: y + 0.9, w: cardW - 0.6, h: cardH - 1.3,
        fontSize: 12, fontFace: "Microsoft YaHei",
        color: this.colors.textLight,
        lineSpacingMultiple: 1.3,
      });
  ...
  addProcessFlowPage(
    ...
    steps.forEach((s, i) => {
      ...
      slide.addText(s.desc, {
        x: 2.6, y: y + 0.45, w: 9.5, h: itemH - 0.5,
        fontSize: 12, fontFace: "Microsoft YaHei",
        color: this.colors.textLight,
      });
    });
  ...
  generateFromOutline(outline: OutlineItem[], contentData: ContentData): void {
    ...
    outline.forEach((page, index) => {
      ...
      switch (effectiveType) {
        ...
        case "toc":
          this.addTOCPage(
            (contentData.toc as Array<{ title: string }>) || [],
          );
          break;
        ...
        case "two-column":
          this.addTwoColumnPage(
            page.title,
            (contentData.leftColumn as string[]) || [],
            (contentData.rightColumn as string[]) || [],
          );
          break;

        case "timeline":
          this.addTimelinePage(
            page.title,
            (contentData.timeline as Array<{
              period: string;
              title: string;
              desc: string;
            }>) || [],
        ...
        case "comparison":
          this.addComparisonPage(
            page.title,
            (contentData.leftSide as { title: string; items: string[] }) || {
              title: "方案A",
              items: [],
            },
            (contentData.rightSide as { title: string; items: string[] }) || {
              title: "方案B",
              items: [],
            },
          );
        ...
        case "icon-grid":
          this.addIconGridPage(
            page.title,
            (contentData.gridItems as Array<{
              icon: string;
              label: string;
              desc: string;
            }>) || [],
        ...
        case "quote":
          this.addQuotePage(
            page.title,
            (contentData.quote as { text: string; author: string }) || {
              text: "",
              author: "",
            },
          );
        ...
        case "card-grid":
          this.addCardGridPage(
            page.title,
            (contentData.cards as Array<{
              title: string;
              desc: string;
              color?: string;
            }>) || [],
        ...
        case "process-flow":
          this.addProcessFlowPage(
            page.title,
            (contentData.steps as Array<{
              step: string;
              title: string;
              desc: string;
            }>) || [],
...
if (require.main === module) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--title" || args[i] === "-t") {
      ...
      const val = args[++i];
      options.layoutStrategy = (val === "mixed" || val === "auto") ? val : "mixed";
      ...
      console.log("可用的排版模式:");
      const layouts = [
        "cover", "toc", "content", "two-column", "timeline",
        "comparison", "icon-grid", "quote", "section-header",
        "card-grid", "process-flow", "big-number", "summary", "end",
      ];
  ...
  const demoOutline: OutlineItem[] = [
    { type: "cover", title: options.title || "演示文稿", subtitle: "混合排版演示" },
    { type: "toc", title: "目录" },
  ...
  generator.generateFromOutline(demoOutline, {
    ...
    timeline: [
      { period: "2024 Q1", title: "项目启动", desc: "完成团队组建与需求调研，确立技术方案选型" },
      { period: "2024 Q2", title: "原型开发", desc: "完成核心功能原型开发，通过内部评审" },
      { period: "2024 Q3", title: "灰度测试", desc: "面向 1000 名种子用户开放测试，收集反馈" },
      { period: "2024 Q4", title: "正式上线", desc: "全量发布，首月用户突破 10 万" },
      { period: "2025 Q1", title: "持续迭代", desc: "根据用户反馈迭代优化，新增 AI 智能功能" },
    ],
    numbers: [
    ...
    cards: [
      { title: "产品研发", desc: "投入 30 人核心研发团队，持续打磨产品体验，每月发布 2 个迭代版本。", color: "1E2761" },
      { title: "市场拓展", desc: "覆盖全国 10 个重点城市，与 50+ 合作伙伴建立战略合作。", color: "58A6FF" },
      { title: "客户成功", desc: "建立 7×24 小时技术支持体系，客户续费率超过 95%。", color: "06D6A0" },
      { title: "生态建设", desc: "开放开发者平台，吸引 100+ 第三方应用接入生态。", color: "B8860B" },
    ],
    quote: {
    ...
    steps: [
      { step: "01", title: "需求分析", desc: "深入了解业务需求，明确目标与范围" },
